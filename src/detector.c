...
//初始化
NetworkHandle yolo_init() {
	//读取配置文件
	printf("loading config file...\n");
	list *options = read_data_cfg("./Config.ini");
	char *name_list = option_find_str(options, "names", "data/names.list");
	char **names = get_labels(name_list);

	char *cfg = option_find_str(options, "cfg", "");
	char *weights = option_find_str(options, "weights", "");
	char *filename = option_find_str(options, "filename", "");

	printf("config file args: name_list=%s , cfg=%s , weights=%s , filename=%s , thresh=%f \n", name_list, cfg, weights, filename);

	network net = parse_network_cfg_custom(cfg, 1);
	if (weights) {
	...
	networkHandle.net = net;
	networkHandle.names = names;

	return networkHandle;
}
//返回值说明：ObjectBoxs
ObjectBoxs yolo_detector(NetworkHandle networkHandle,char *filename, float thresh) {
	network net = networkHandle.net;
	char **names = networkHandle.names;
	clock_t time;
	char buff[256];
	char *input = buff;
	int j;
